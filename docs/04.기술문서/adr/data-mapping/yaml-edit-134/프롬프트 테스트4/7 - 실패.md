...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Use 'data' instead of 'stringData' for Secret values, and ensure values are Base64 encoded.
data:
...
data:
  ...
  # [FIXED] Corrected KAKAO_REDIRECT_URI to match the provided context.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected FRONT_DOMAIN to match the provided context.
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected port from 8094 to 8084 to match the service definition.
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
