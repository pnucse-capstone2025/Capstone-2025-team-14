...
# [FIXED] Moved Secret values from `stringData` to `data` as per policy, and removed `stringData` usage.
---
apiVersion: v1
kind: ConfigMap
metadata:
...
# [FIXED] Corrected `KAKAO_REDIRECT_URI` and `FRONT_DOMAIN` values to match the specification.
---
apiVersion: apps/v1
kind: Deployment
metadata:
      ...
      - name: user-service
        ...
        resources:
          ...
          limits:
            cpu: "1000m"
            memory: "1Gi"
# [FIXED] Corrected deployment name and matchLabels to `user-service`.
---
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  selector:
    app: user-service
# [FIXED] Corrected service name and port to match the specification.
---
apiVersion: apps/v1
kind: Deployment
metadata:
      ...
      - name: order-service
        ...
        resources:
          ...
          limits:
            cpu: "700m"
            memory: "1Gi"
# [FIXED] Added missing environment variables for `order-service`.
---
apiVersion: v1
kind: Service
metadata:
      ...
      - name: gift-app
        ...
        resources:
          ...
          limits:
            cpu: "500m"
            memory: "1Gi"
# [FIXED] Added missing environment variable `KAKAO_CLIENT_SECRET` for `gift-app`.
---
apiVersion: v1
kind: Service
metadata:
