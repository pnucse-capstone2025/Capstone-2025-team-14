...
metadata:
  ...
  labels:
    ...
    tier: backend
data:
  # [FIXED] Changed from 'stringData' to 'data' as per policy and base64 encoded values are required.
...
data:
  ...
  # [FIXED] Corrected the KAKAO_REDIRECT_URI as per the specification.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected the FRONT_DOMAIN as per the specification.
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected service port to match the specification.
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
