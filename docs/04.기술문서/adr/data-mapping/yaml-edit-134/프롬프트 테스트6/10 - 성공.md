...
metadata:
  ...
  labels:
    ...
    tier: backend
data: # [FIXED] Changed from stringData to data for security compliance
...
metadata:
  ...
  labels:
    app: user-service # [FIXED] Corrected label from member-service to user-service
    project: gift-service
    tier: backend
spec:
  replicas: 1
    ...
    spec:
      ...
      - name: user-service
        ...
        - secretRef:
            name: gift-secret
        env:
        - name: KAKAO_REDIRECT_URI # [FIXED] Added missing environment variable
          valueFrom:
            configMapKeyRef:
              name: gift-config
...
kind: Service
metadata:
  name: user-service-svc # [FIXED] Corrected name from member-service-svc to user-service-svc
  labels:
    app: user-service # [FIXED] Corrected label from member-service to user-service
    project: gift-service
    tier: backend
