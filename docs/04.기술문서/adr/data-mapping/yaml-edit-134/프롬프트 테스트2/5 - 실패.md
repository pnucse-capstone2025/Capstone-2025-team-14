...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Secret 값은 stringData가 아닌 data 필드에 저장해야 합니다.
data:
...
data:
  ...
  # [FIXED] KAKAO_REDIRECT_URI의 경로가 잘못되어 수정합니다.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] FRONT_DOMAIN은 8080 포트를 사용해야 합니다.
...
kind: Deployment
metadata:
  # [FIXED] 서비스 이름이 잘못되었습니다. user-service로 수정합니다.
  name: user-service
  labels:
    app: user-service
    project: gift-service
...
kind: Service
metadata:
  # [FIXED] 서비스 이름이 잘못되었습니다. user-service-svc로 수정합니다.
