...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Changed stringData to data for sensitive information as per policy.
data:
...
data:
  ...
  # [FIXED] Corrected the KAKAO_REDIRECT_URI path to match the context.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Updated FRONT_DOMAIN to match the expected port.
...
kind: Deployment
metadata:
  # [FIXED] Corrected the name from member-service to user-service to match context.
  name: user-service
  labels:
    app: user-service
    project: gift-service
...
kind: Service
metadata:
  # [FIXED] Corrected the name from member-service-svc to user-service-svc to match context.
...
spec:
  type: ClusterIP
  ports:
  - # [FIXED] Corrected the port from 8094 to 8084 to match the service definition.
    port: 8084
    targetPort: 8084
  selector:
    app: user-service
        ...
        - name: KAKAO_CLIENT_ID
          valueFrom:
            secretKeyRef:
              name: gift-secret
              key: KAKAO_CLIENT_ID
        # [FIXED] Added missing environment variable KAKAO_CLIENT_SECRET.
        - name: KAKAO_CLIENT_SECRET
          valueFrom:
            secretKeyRef:
              name: gift-secret
