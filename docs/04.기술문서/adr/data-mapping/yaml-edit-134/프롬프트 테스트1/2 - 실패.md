...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Secret에서 stringData 사용 금지. 데이터를 data 필드로 이동
data:
...
# [FIXED] KAKAO_REDIRECT_URI 수정: 잘못된 URI 경로 수정
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] FRONT_DOMAIN 수정: 잘못된 포트 수정
...
metadata:
  name: user-service
# [FIXED] Deployment 이름 수정: 잘못된 이름(member-service) 수정
  labels:
    app: user-service
    project: gift-service
    tier: backend
...
# [FIXED] Service 이름 수정: 잘못된 이름(member-service-svc) 수정
  labels:
    app: user-service
    project: gift-service
    tier: backend
