...
| ----------- | ---------- | ----------------------------------------------------- |
| —           | —          | 제공된 YAML에는 `# From source_X` 형식의 주석이 없어 매핑할 항목이 없습니다. |


## 특징: configMap과 Secret에서 data필드가 metadata의 하위 필드로 들어감. 이러면 안되고 최상위 필드로 나와야 함. data 필드만 최상위 필드로 빼주니 배포 다 되고, 모든 서비스도 정상 동작함.