...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Changed from `stringData` to `data` as per policy, encoded values.
data:
...
# [FIXED] Corrected the URI path for KAKAO_REDIRECT_URI.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] Corrected the FRONT_DOMAIN value.
...
metadata:
  ...
  labels:
    ...
    project: gift-service
    tier: backend
# [FIXED] Corrected the metadata.name and matchLabels.app to `user-service`.
spec:
  replicas: 1
  selector:
    matchLabels:
...
metadata:
  ...
  labels:
    ...
    project: gift-service
    tier: backend
# [FIXED] Corrected the service name and selector to match `user-service`.
spec:
  type: ClusterIP
  ports:
  - port: 8084
