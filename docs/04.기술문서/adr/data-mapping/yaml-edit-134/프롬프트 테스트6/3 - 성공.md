...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Changed from 'stringData' to 'data' as per policy
data:
...
# [FIXED] Corrected the KAKAO_REDIRECT_URI path
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] Corrected the FRONT_DOMAIN to match the specification
...
metadata:
  ...
  labels:
    ...
    project: gift-service
    tier: backend
# [FIXED] Corrected 'member-service' to 'user-service' in metadata and selector
spec:
  replicas: 1
  selector:
    matchLabels:
...
metadata:
  ...
  labels:
    ...
    project: gift-service
    tier: backend
# [FIXED] Corrected 'member-service-svc' to 'user-service-svc'
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected the port to match the service specification
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
