...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Secret의 데이터는 stringData가 아닌 data 필드에 Base64 인코딩된 값을 사용하도록 수정
data:
...
data:
  ...
  # [FIXED] KAKAO_REDIRECT_URI의 URL 경로 수정
...
kind: Deployment
metadata:
  # [FIXED] 잘못된 이름을 user-service로 수정
  name: user-service
  labels:
    # [FIXED] 잘못된 레이블을 user-service로 수정
    app: user-service
    project: gift-service
    tier: backend
spec:
...
kind: Service
metadata:
  # [FIXED] 잘못된 이름을 user-service-svc로 수정
