...
metadata:
  ...
  labels:
    ...
    tier: backend
data:
  # [FIXED] stringData 사용을 금지하고 data 필드를 사용해야 합니다.
...
metadata:
  name: user-service
  # [FIXED] deployment 이름 수정: member-service -> user-service (정답지와 일치)
  labels:
    app: user-service
    project: gift-service
    tier: backend
...
metadata:
  ...
  # [FIXED] 서비스 이름 및 포트 수정: member-service-svc -> user-service-svc, 8094 -> 8084
  labels:
    app: user-service
    project: gift-service
    tier: backend
  ...
  template:
    ...
    spec:
      ...
      - name: order-service
        ...
        # [FIXED] 누락된 환경 변수 추가: SERVICE_WISH_URI
        ...
        - name: KAKAO_CLIENT_ID
          valueFrom:
            secretKeyRef:
              name: gift-secret
              key: KAKAO_CLIENT_ID
        # [FIXED] 누락된 환경 변수 추가: KAKAO_CLIENT_SECRET
        - name: KAKAO_CLIENT_SECRET
          valueFrom:
            secretKeyRef:
              name: gift-secret
