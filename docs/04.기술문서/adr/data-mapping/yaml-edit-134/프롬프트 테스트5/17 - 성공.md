...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Changed stringData to data to comply with Secret management policy
data:
...
# [FIXED] Corrected URI path from /members/login/oauth/code/kakao to /members/login/oauth2/code/kakao
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] Corrected domain from http://localhost:8085 to http://localhost:8080
...
metadata:
  ...
  labels:
    ...
    project: gift-service
    tier: backend
# [FIXED] Changed name from member-service to user-service for consistency
spec:
  replicas: 1
  selector:
    matchLabels:
...
metadata:
  ...
  labels:
    ...
    project: gift-service
    tier: backend
# [FIXED] Changed name from member-service-svc to user-service-svc for consistency
spec:
  type: ClusterIP
  ports:
  - port: 8084
