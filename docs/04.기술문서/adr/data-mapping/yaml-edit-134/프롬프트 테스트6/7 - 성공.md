...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Removed `stringData` as it is not allowed. Using `data` instead.
data:
...
# [FIXED] Corrected KAKAO_REDIRECT_URI to match the specification.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] Corrected FRONT_DOMAIN to match the specification.
...
kind: Deployment
metadata:
  # [FIXED] Corrected the metadata name from `member-service` to `user-service`.
  name: user-service
  labels:
    app: user-service
    project: gift-service
...
kind: Service
metadata:
  # [FIXED] Corrected the metadata name from `member-service-svc` to `user-service-svc`.
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected the port to match the specification.
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
