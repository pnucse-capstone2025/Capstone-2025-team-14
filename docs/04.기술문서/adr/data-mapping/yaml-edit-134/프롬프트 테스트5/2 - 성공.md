...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Moved data from stringData to data to comply with the policy.
data:
...
data:
  ...
  # [FIXED] Corrected KAKAO_REDIRECT_URI to match the source of truth.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected FRONT_DOMAIN to match the source of truth.
...
metadata:
  name: user-service
  # [FIXED] Corrected metadata.name and labels.app to match source of truth.
  labels:
    app: user-service
    project: gift-service
    tier: backend
...
metadata:
  ...
  # [FIXED] Corrected service name to match the source of truth.
  labels:
    app: user-service
    project: gift-service
    tier: backend
spec:
  ...
  ports:
  - port: 8084
    # [FIXED] Corrected port to match the source of truth.
    targetPort: 8084
  selector:
    app: user-service
---
  ...
  template:
    ...
    spec:
      ...
      - name: order-service
        ...
        # [FIXED] Added missing environment variable SERVICE_WISH_URI.
        - name: FRONT_DOMAIN
          valueFrom:
            configMapKeyRef:
              name: gift-config
