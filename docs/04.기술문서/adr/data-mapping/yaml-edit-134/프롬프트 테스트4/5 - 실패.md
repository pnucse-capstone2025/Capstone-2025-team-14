...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] stringData 사용 금지 정책에 따라 stringData를 data로 변경하고 Base64 인코딩된 값을 사용했습니다.
data:
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] 포트 번호 수정: 잘못된 포트 번호 8094를 8084로 수정했습니다.
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
