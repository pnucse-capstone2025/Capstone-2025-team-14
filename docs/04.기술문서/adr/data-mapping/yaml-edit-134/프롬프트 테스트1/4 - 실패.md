...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] stringData 사용 금지 규칙에 따라 data 필드로 변경
data:
...
# [FIXED] KAKAO_REDIRECT_URI 값 오탈자 수정
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] FRONT_DOMAIN 값 수정: 8085에서 8080으로 변경
...
metadata:
  name: user-service
  # [FIXED] 잘못된 이름 'member-service'를 'user-service'로 수정
  labels:
    app: user-service
    project: gift-service
    tier: backend
...
metadata:
  ...
  # [FIXED] 서비스 이름을 'member-service-svc'에서 'user-service-svc'로 수정
  labels:
    app: user-service
    project: gift-service
    tier: backend
spec:
  type: ClusterIP
  ports:
  # [FIXED] 잘못된 포트 번호 8094를 8084로 수정
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
