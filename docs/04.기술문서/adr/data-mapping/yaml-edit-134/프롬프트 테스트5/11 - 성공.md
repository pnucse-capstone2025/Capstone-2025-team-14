...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Moved Secret data from `stringData` to `data` and ensured values are Base64 encoded
data:
...
# [FIXED] Corrected KAKAO_REDIRECT_URI to match the context specification
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] Corrected FRONT_DOMAIN to match the context specification
