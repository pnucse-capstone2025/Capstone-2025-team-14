...
metadata:
  ...
  labels:
    ...
    tier: backend
data:  # [FIXED] Changed from 'stringData' to 'data' as per security policy
...
data:
  ...
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao  # [FIXED] Corrected the URI path
  FRONT_DOMAIN: http://localhost:8080  # [FIXED] Corrected to match the specified domain
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
kind: Deployment
metadata:
  name: user-service  # [FIXED] Corrected the name to match the service purpose
  labels:
    app: user-service  # [FIXED] Corrected the label to match the service name
    project: gift-service
    tier: backend
spec:
  replicas: 1
...
kind: Service
metadata:
  name: user-service-svc  # [FIXED] Corrected the name to match the service purpose
  labels:
    app: user-service  # [FIXED] Corrected the label to match the service name
    project: gift-service
    tier: backend
spec:
  type: ClusterIP
  ports:
  - port: 8084  # [FIXED] Corrected the port to match the service definition
    targetPort: 8084
  selector:
    app: user-service
---
