...
metadata:
  ...
  labels:
    ...
    tier: backend
data:
  # [FIXED] Corrected field from stringData to data, as per policy to not use stringData.
...
data:
  ...
  # [FIXED] Corrected KAKAO_REDIRECT_URI based on the context specification.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected FRONT_DOMAIN based on the context specification.
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected port to match internal service port as per context specification.
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
