...
metadata:
  name: gift-secret
  labels:
    app: gift-service # [FIXED] 올바른 마이크로서비스 이름으로 라벨 수정
    project: gift-service
    tier: backend
data: # [FIXED] stringData 사용 금지, data 필드로 변경
...
data:
  ...
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao # [FIXED] 잘못된 경로 수정
  FRONT_DOMAIN: http://localhost:8080 # [FIXED] 잘못된 포트 수정
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
kind: Deployment
metadata:
  name: user-service # [FIXED] 잘못된 이름 수정
  labels:
    app: user-service # [FIXED] 올바른 마이크로서비스 이름으로 라벨 수정
    project: gift-service
    tier: backend
spec:
  replicas: 1
...
kind: Service
metadata:
  name: user-service-svc # [FIXED] 잘못된 이름 수정
  labels:
    app: user-service # [FIXED] 올바른 마이크로서비스 이름으로 라벨 수정
    project: gift-service
    tier: backend
        ...
        - name: SERVICE_PRODUCT_URI
          valueFrom:
            configMapKeyRef:
              name: gift-config
              key: SERVICE_PRODUCT_URI
        - name: SERVICE_WISH_URI # [FIXED] 누락된 환경 변수 추가
          valueFrom:
            configMapKeyRef:
              name: gift-config
        ...
        - name: KAKAO_CLIENT_ID
          valueFrom:
            secretKeyRef:
              name: gift-secret
              key: KAKAO_CLIENT_ID
        - name: KAKAO_CLIENT_SECRET # [FIXED] 누락된 환경 변수 추가
          valueFrom:
            secretKeyRef:
              name: gift-secret
              key: KAKAO_CLIENT_SECRET
