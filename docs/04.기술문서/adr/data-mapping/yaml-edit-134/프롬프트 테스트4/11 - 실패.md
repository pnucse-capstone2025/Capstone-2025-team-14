...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Secret should not use `stringData`, corrected to `data` with base64 encoded values.
data:
...
# [FIXED] Corrected KAKAO_REDIRECT_URI to match the defined context.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
# [FIXED] Corrected FRONT_DOMAIN to match the defined context.
...
metadata:
  name: user-service
# [FIXED] Corrected the name to `user-service` to match the application name in the context.
  labels:
    app: user-service
    project: gift-service
    tier: backend
...
# [FIXED] Corrected the service name to `user-service-svc` to match the context.
  labels:
    app: user-service
    project: gift-service
    tier: backend
spec:
  ...
  ports:
  - port: 8084
# [FIXED] Corrected the port to match the defined user-service port in the context.
    targetPort: 8084
  selector:
    app: user-service
---
