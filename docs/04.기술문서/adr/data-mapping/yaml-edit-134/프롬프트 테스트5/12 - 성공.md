...
metadata:
  ...
  labels:
    ...
    tier: backend
data:
  # [FIXED] Moved from `stringData` to `data` to comply with security policy.
...
data:
  ...
  # [FIXED] Corrected KAKAO_REDIRECT_URI value as per the source of truth.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected FRONT_DOMAIN value as per the source of truth.
