...
metadata:
  ...
  labels:
    ...
    tier: backend
data:
  # [FIXED] Changed from stringData to data as per policy; values must be Base64 encoded under data
...
data:
  ...
  # [FIXED] Corrected the URI path to match the answer key
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected the FRONT_DOMAIN to match the answer key
...
kind: Deployment
metadata:
  # [FIXED] Corrected metadata.name to match the correct service name
  name: user-service
  labels:
    # [FIXED] Corrected app label to match the correct service name
    app: user-service
    project: gift-service
    tier: backend
spec:
  ...
  selector:
    matchLabels:
      # [FIXED] Corrected selector to match the correct app label
      app: user-service
  template:
    metadata:
      labels:
...
kind: Service
metadata:
  # [FIXED] Corrected service name to match the correct service name
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected port to match the correct service port
  - port: 8084
    targetPort: 8084
  selector:
    # [FIXED] Corrected selector to match the correct app label
  ...
  template:
    ...
    spec:
      ...
      - name: order-service
        ...
        # [FIXED] Added missing environment variable SERVICE_WISH_URI from the configMap
        ...
        - name: KAKAO_CLIENT_ID
          valueFrom:
            secretKeyRef:
              name: gift-secret
              key: KAKAO_CLIENT_ID
        # [FIXED] Added missing environment variable KAKAO_CLIENT_SECRET from the secret
        - name: KAKAO_CLIENT_SECRET
          valueFrom:
            secretKeyRef:
              name: gift-secret
