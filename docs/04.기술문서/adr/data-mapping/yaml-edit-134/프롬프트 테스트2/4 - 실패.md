...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] 잘못되어 있던 stringData를 data로 수정했습니다.
data:
...
data:
  ...
  # [FIXED] 잘못되어 있던 KAKAO_REDIRECT_URI 값을 수정했습니다.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] 잘못되어 있던 FRONT_DOMAIN 값을 수정했습니다.
...
kind: Deployment
metadata:
  # [FIXED] 잘못된 이름을 수정했습니다. member-service -> user-service
  name: user-service
  labels:
    # [FIXED] 잘못된 레이블을 수정했습니다. member-service -> user-service
    app: user-service
    project: gift-service
    tier: backend
spec:
...
kind: Service
metadata:
  # [FIXED] 잘못된 서비스 이름을 수정했습니다. member-service-svc -> user-service-svc
