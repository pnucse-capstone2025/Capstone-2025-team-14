...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Secret는 stringData 대신 data를 사용해야 함
data:
...
kind: Deployment
metadata:
  # [FIXED] Deployment 이름을 user-service로 수정
  name: user-service
  labels:
    # [FIXED] 레이블의 app 값을 user-service로 수정
    app: user-service
    project: gift-service
    tier: backend
spec:
...
kind: Service
metadata:
  # [FIXED] Service 이름을 user-service-svc로 수정
  ...
  template:
    ...
    spec:
      ...
      - name: order-service
        ...
        # [FIXED] 누락된 SERVICE_WISH_URI 환경 변수 추가
