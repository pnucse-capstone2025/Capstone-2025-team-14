...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Secret은 stringData 대신 data 필드를 사용해야 합니다.
data:
...
data:
  ...
  # [FIXED] KAKAO_REDIRECT_URI가 잘못된 URL로 설정되어 있어 수정하였습니다.
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] FRONT_DOMAIN이 잘못된 URL로 설정되어 있어 수정하였습니다.
...
kind: Deployment
metadata:
  # [FIXED] Deployment 이름과 matchLabels.app 값이 일치하지 않아 수정하였습니다.
  name: user-service
  labels:
    app: user-service
    project: gift-service
...
kind: Service
metadata:
  # [FIXED] Service 이름이 잘못되어 수정하였습니다.
...
## 비고: order-service에서 필요한 환경변수 주입 누락