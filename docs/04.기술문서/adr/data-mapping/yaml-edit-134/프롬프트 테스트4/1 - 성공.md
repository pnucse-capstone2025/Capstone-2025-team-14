...
metadata:
  ...
  labels:
    ...
    tier: backend
# [FIXED] Changed from stringData to data for Secret values to ensure Base64 encoding compliance.
data:
...
data:
  ...
  # [FIXED] Corrected typo in KAKAO_REDIRECT_URI path
  KAKAO_REDIRECT_URI: http://localhost:8080/members/login/oauth2/code/kakao
  # [FIXED] Corrected the FRONT_DOMAIN to match the context
...
spec:
  type: ClusterIP
  ports:
  # [FIXED] Corrected the port from 8094 to 8084 to match internal context
  - port: 8084
    targetPort: 8084
  selector:
    app: user-service
